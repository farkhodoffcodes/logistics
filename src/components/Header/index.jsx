...
const Header = () => {
  ...
  return (
    <header className='flex top-0 z-50 justify-between w-full pr-10 items-center fixed py-4 bg-white max-sm:fixed max-sm:full'>
      <div className='flex gap-5 ml-28'>
        <img className='w-auto h-10 max-sm:hidden' src={logo} alt='logo' />

        <i
          onClick={toggleFuc}
          className={`fa-solid fa-bars sm:hidden max-sm:relative`}
        ></i>
      ...
      <div className='flex items-center justify-center gap-5 max-sm:absolute max-sm:ml-5'>
        ...
        <nav className={toggle ? 'block' : 'hidden'}>
          <div className='max-sm:mt-10'>
            <i
              onClick={toggleFuc}
              className='fa-solid fa-xmark sm:hidden max-sm:ml-20'
            ></i>
